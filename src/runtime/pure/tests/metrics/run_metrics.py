...
def main() -> int:
    ...
    # Primary speed metric: same scalar loop pure vs host (blog / GSoC win)
    ...
    ratio_elem = times.get("ratio_pure_elem_array_over_host_array", pure_e / host_a if host_a else float("nan"))
    ratio_simd = times.get("ratio_pure_simd_v_over_host_array", pure_v / host_a if host_a else float("nan"))
    ratio_bindc = times.get("ratio_pure_bindc_over_host_scalar", float("nan"))
    ratio_best = min(ratio_scalar_loop, ratio_best_array)  # either path under 1 is the win
    ...
    md = f"""# Pure math backend metrics (SIMD-enabled)

Generated by `src/runtime/pure/tests/metrics/run_metrics.py` on **rg.terra**.

## Method

| Item | Value |
|------|--------|
| Accuracy grid | x ∈ [-20, 20], n = {n_grid} |
| Primary timing | n = 1e7 pure `_lfortran_pure_dsin` vs host bind(c) `sin` on [0, 1.5] |
| Array timing | n = 1e6 in [-1.5, 1.5], reps = 200 (host array-expr may use libmvec) |
| Flags | `{" ".join(gflags)}` |
| Note | Vectorization **enabled** (no `-fno-tree-vectorize`) |
...
|------|---------|-----------------|
| pure `dsin(xx)` loop | {pure_sl:.6e} | {ratio_scalar_loop:.3f} |
| host `sin(xx)` loop | {host_sl:.6e} | 1.000 |
| pure bind(c) `_lfortran_pure_dsin` | {times.get("pure_bindc_scalar", float("nan")):.6e} | {ratio_bindc:.3f} |
| pure ns/call | {times.get("pure_ns_per_call", float("nan")):.3f} | |
| host bind(c) `sin` ns/call | {times.get("host_ns_per_call", float("nan")):.3f} | |

### Secondary — array sin intrinsic (`yp = dsin(x)` vs `yh = sin(x)`)

| Path | seconds | ratio vs host array-expr |
|------|---------|--------------------------|
| host array-expr `sin(x)` | {host_a:.6e} | 1.000 |
| pure elemental `yp = dsin(x)` | {pure_e:.6e} | {ratio_elem:.3f} |
| pure elemental `yp=dsin(x)` (alias) | {pure_v:.6e} | {ratio_simd:.3f} |
| pure bind(c) batch | {times.get("pure_bindc_v", float("nan")):.6e} | {(times.get("pure_bindc_v", float("nan"))/host_a) if host_a else float("nan"):.3f} |
