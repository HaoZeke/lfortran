...
| Primary timing | n = 5e6 same scalar loop pure `dsin` vs host `sin` on [0, 1.5] |
...
### Primary — same scalar loop (principal band, blog metric)
...
### Secondary — array (host may use libmvec on `yh = sin(x)`)
...
| pure auto-vec `dsin_v` | {pure_v:.6e} | {ratio_simd:.3f} |
